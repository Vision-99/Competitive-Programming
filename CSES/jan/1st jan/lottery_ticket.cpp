...
void I_still_love_her() {
    ...
    for(ll i = 1; i < n; i++) {
        ...
        if(chef < other) {
            ...
            intervals.pb({mid+1, 1000000});
        } else {
            intervals.pb({mid, mid});
        }
    }
    ...
    for(auto [l, r] : intervals) {
        if(l > currEnd + 1) break;
        currEnd = max(currEnd, r);
    }
    
    ans = min(currEnd, 1000000LL) - intervals[0].first + 1;
    cout << max(0LL, ans);
}
