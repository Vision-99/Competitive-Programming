...
void I_still_love_her(){
    ...
    cin >> s;
    ll n = s.size();
    ll ans = n; // worst case: remove all digits
    
    // Check for all possible pairs that make number divisible by 25
    vector<string> div = {"00", "25", "50", "75"};
    
    for(string d : div) {
        // Find last occurrence of second digit
        for(int i = n-1; i >= 0; i--) {
            if(s[i] == d[1]) {
                // Find last occurrence of first digit before i
                for(int j = i-1; j >= 0; j--) {
                    if(s[j] == d[0]) {
                        // digits to remove = total digits - 2 (the two digits we keep)
                        ans = min(ans, n - j - 2);
                    }
                }
            }
