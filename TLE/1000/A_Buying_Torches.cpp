...
/*
    need k torch
    1 torch need 1 stick and 1 coal

    1 stick -> x sticks
    y sticks -> 1 coal

    need k coal , for that need y*k sticks and also k sticks, total stick k*(y+1)

    1st need 1->k*(y+1) type 1 trade
    then need to add k ( cz it's the type  2 trade)

    type 1->
        Each operation lets you replace 1 stick with x sticks, so the total number of sticks increases by:    x-1

        after z ops , toatl sticks = N = 1 + z*(x-1)
        need atleast N , 
