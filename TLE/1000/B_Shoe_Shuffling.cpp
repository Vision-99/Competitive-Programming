...
/*
    mp[x]==1 not allowed
    find same len seg , and shift the iota wrt that
    
*/

void shift(vi &a,int l,int r){
    int temp = a[l];
...
void I_still_love_her() {
    ...
    for(int i=0;i+1<z;i++){
        if(i==0){
            shift(ans,0,flip_ind[i+1]);
        }
        else if(i==z-2){
            shift(ans,flip_ind[i]+1,n-1);
        }
        else{
            shift(ans,flip_ind[i]+1,flip_ind[i+1]);
        }
    }
