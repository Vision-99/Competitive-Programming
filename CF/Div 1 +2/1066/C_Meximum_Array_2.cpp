...
/*
    for each Query , 
    if c==1 , min of subarray from l to r== k
    if c==2, Mex. of subarray from l to r== k

    find such array of size and print it.

    for c=1 ,0 must not be in range , as k>=1
    and for c=2, we must have ele from (0,1,2,...k-2,k-1,k)   So, r-l+1>= k ( given also)
