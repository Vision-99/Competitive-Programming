...
void I_still_love_her() {
    ...
    int msb_a=log2(a) +1;
    int msb_b=log2(b) +1;
    if(msb_a < msb_b ) {cout<<-1;return;}



    int cnt=0;

    // a----> b , set in a, unset in b, so plz unset that bit in a, by taking xor
    // with 1<<bit

    // unset in a , set in b, set in a by taking xor with 1<<bit
    vi temp;
