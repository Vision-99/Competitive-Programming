...
void I_still_love_her() {
    ...
    vector<char> pres(n+1,0);

    fl(i,0,n) if(a[i]>=1 && a[i]<=n) pres[a[i]] = 1;
    vi unused;
    for(int v=1; v<=n; ++v) if(!pres[v]) unused.pb(v);
    vi ans;
    for(int i=0;i<unused.size() && (int)ans.size()<k; i++) ans.pb(unused[i]);

    int f = a[0];

    while((int)ans.size() < k) ans.pb(f);
