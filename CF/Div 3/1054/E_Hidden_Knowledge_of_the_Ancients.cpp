...
void I_still_love_her() {
    ...
    ll R=-1,R1=-1,d0=0,d1=0,ans=0;

    fl(i,0,n){
        while (R+1<n) {
            ll x=v[R+1];
            if (c0.find(x)==c0.end() && d0==k) break;
            R++;
            if (c0[x]==0) d0++;
            c0[x]++;
        }
        while (R1+1<n) {
            ll x=v[R1+1];
            if (c1.find(x)==c1.end() && d1==(k-1)) break;
            R1++;
            if (c1[x]==0) d1++;
            c1[x]++;
        }
        ll le=i+l-1, ri=i+r-1;
        ll low=max(R1+1,le);
        ll high=min(R,ri);
