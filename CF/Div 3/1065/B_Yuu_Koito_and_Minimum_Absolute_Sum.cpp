...
/*
    ...
    -1 to 0 , if n==2 and all not -1 then make them eqaul
    it's just fuking 1st and last ele.
    midel -1 can't do shit
*/
...
void I_still_love_her() {
    ...
    for(int i=1;i+1<n;i++) if(a[i]==-1) a[i]=0;

    if(a[0]==-1 and a.back()==-1) a[0]=0,a[n-1]=0;
    else if(a[0]==-1 and a.back()!=-1) a[0]=a[n-1];
    else if(a[0]!=-1 and a.back()==-1) a[n-1]=a[0];

    int sher=0;

    for(int i=0;i+1<n;i++) sher += a[i+1] - a[i];
    cout<<abs(sher)<<'\n';
