...
/*
    ...
    array ele can get upto 1e6
    on ith move swap(a[i],b[i]);

    final score is Xor of all ele
    Alice -> f(a); , f()- xor of array
    mai -> f(b)

    if(f(a) > f(b)) Alice win
    else if( f(a) < f(b) ) Mai win
    else Tie;

    it's all depends on the wather so Swap or not
    moves are from 1 to n ,
    on odd move Alice take turn , either swap or skip
    on even move Mai do stuff.

    it's XOR and ele are from 0 to 1e6, 21 bits 0 to 20, req. to rep. 
    think in binary, 
    SO in Xor max answer you can get is Vmax , nah wrong thinking

    make a delta ,d ^=a[i]^b[i]
    if swap f(a)^d
