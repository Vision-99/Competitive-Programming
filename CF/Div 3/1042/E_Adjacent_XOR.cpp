...
void I_still_love_her() {
    ...
    // WE  just have to find the order in which we need to apply 
    // on which ind , that's clear (where values are not equal)
    
    if(a.back()!=b.back()) NR
    fl(i,0,n-1){
        if(a[i]==b[i] or ((a[i]^a[i+1]) == b[i]) or ((a[i]^b[i+1]) == b[i])) continue;
        else NR
    }
