...
/*
    it's just bf
    find a num that is both prime and passes the check
    cz product of just 1st 17 prime is bigger then 1e18
    ( fast way to do asume all prime of oder 10) so just 18 nums , will give you 1e18
   
*/

bool check(vi & a,int x){
    for(auto e:a) if(e%x) return 1;
    return 0;
}
...
void I_still_love_her() {
    ...
    while(!(isprime(ans) and check(a,ans))) ans++;
