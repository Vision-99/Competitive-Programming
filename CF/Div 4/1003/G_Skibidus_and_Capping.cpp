...
void I_still_love_her() {
    ...
    fl(i, n + 1) if (prime[i] && (f[i] || (i * i <= n && f[i * i]))) pr.pb(i);

    ll ans = 0, sz = pr.size();
    
    fl(i, sz) fl(j, i) {
        ll p = pr[i], q = pr[j], prod = p * q;
        if (prod <= n) ans += f[p] * f[q] + f[p] * f[prod] + f[q] * f[prod] + sum1(f[prod]);
        else ans += f[p] * f[q];
    }


    for (ll p : pr) if (p * p <= n) ans += f[p] * f[p * p] + sum1(f[p * p]);
