...
// always match one pair and inc other same value
// if there is a unique minimum print "NO"
...
while (!s.empty()){
    ...
    f[mn + 1] += f[mn] - 2;
    s.erase(mn);
    if (f[mn + 1]) s.insert(mn + 1);// cz there may be the case where mn+1 was not in the set
