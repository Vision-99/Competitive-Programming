...
void I_still_love_her() {
    ...
    if (n == 0 || s.empty()) { // Handle edge case
        cout << s << endl;
        return;
    }
    ...
    char highestChar = '\0', lowestChar = '\0';
    ll maxFreq = 0, minFreq = INT_MAX;

    
    for (auto &x : mp) {
        if (x.second > maxFreq || (x.second == maxFreq && x.first > highestChar)) {
            maxFreq = x.second;
            highestChar = x.first;
        }
        if (x.second < minFreq || (x.second == minFreq && x.first < lowestChar)) {
            minFreq = x.second;
            lowestChar = x.first;
        }
    }
