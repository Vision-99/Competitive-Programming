...
/*
    ...
    CF= edu 183.C "Monocrap string"

    treat 'a' as +1 and 'b' as -1 ,need sum of array to be zero , if already zero print 0
    otherwise we need to remove the smallest subarray with Vsum(i.e. orignal sum of array)
...
void I_still_love_her() {
    ...
    // now find minimum size of subarray such that it's sum is 'k'
    // now it is just typical leetcode

    unordered_map<int,int> mp; // pSum,ind

    int pSum=0,ans=1e9;

    for(int i=0;i<n;i++){
        pSum += a[i];
        if(pSum==k) ans = min(ans,i+1); // subarray starting with 0 ind.

        int rem  = pSum - k;
        if(mp.count(rem)) ans = min(ans,i-mp[rem]);
        mp[pSum]=i; // put recent value for min size
    }



    if(ans==1e9 or ans==n) cout<<-1<<'\n'; // ans==n edge case where i have to remove all ele.
    else cout<<ans<<'\n'; 
