...
/*
    ...
    just make binary of n , and check for palindrome by removing last zeros
    and if size of remainging is odd s[n/2] must be zero , same on xro gives zero


*/

string int_to_binary_with_no_back_zero(int n){
    string ans;
    while(n){
        ans = (char)('0'+ (n&1)) + ans;
        n>>=1;
...
void I_still_love_her() {
    ...
    cin>>n;
    if(n==0) YR
    string s=int_to_binary_with_no_back_zero(n);

    // if not pal rerun NO

    if(!pal(s)) NR

    // if s.size() is odd, middle char must be zero
    int sz = s.size();
    if(sz &1){
        if(s[sz/2]!='0') NR
    }
