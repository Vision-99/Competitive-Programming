...
bool sex_internal_cum(vi a,vi b){
    if(max(a[0],a[1])==max(b[0],b[1]))    return (min(a[0],a[1])<min(b[0],b[1]));
    else return (max(a[0],a[1])<max(b[0],b[1]));
}
...
vector<vi> a(n,vector<ll>(2,0));

for(ll i=0;i<n;i++){
    cin>>a[i][0]>>a[i][1];
}

sort(all(a),sex_internal_cum);
