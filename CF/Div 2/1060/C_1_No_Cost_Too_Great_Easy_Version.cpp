...
/* Precompute SPF globally up to MAXV */
...
vector<ll> factor_primes_unique(int x) {
    // returns unique prime factors of x using SPF (x <= MAXV)
    vector<ll> res;
    while (x > 1) {
        int p = SPF[x];
        res.pb(p);
...
/*
    Fixed solution using macros and helpers.
*/
void I_still_love_her() {
    ...
    // count prime divisibilities among original a[]
    unordered_map<ll,int> cntp;
    vector<vector<ll>> pf(n);
    fl(i,0,n){
        pf[i] = factor_primes_unique((int)a[i]);
        for (auto p: pf[i]) cntp[p]++;
    }

    // if some prime divides >=2 original numbers -> 0
    for (auto &pr: cntp) if (pr.second >= 2) { cout << 0; return; }

    // check if increasing some a[i] by 1 creates a common prime with some other original number
    fl(i,0,n){
        int val = (int)(a[i] + 1);
        vector<ll> primes = factor_primes_unique(val);
        for (auto p: primes){
            int oth = cntp.count(p) ? cntp[p] : 0;
            // if a[i] itself was divisible by p, don't count it as "other"
            bool self_has = false;
            // pf[i] is small, linear search is fine
            for (auto q: pf[i]) if (q == p) { self_has = true; break; }
            if (self_has) oth -= 1;
            if (oth >= 1) { cout << 1; return; }
        }
    }
