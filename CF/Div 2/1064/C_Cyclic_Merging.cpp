...
/*
    ...
    ans += max(a[i],a[i+1])  
    a[i] = max(a[i],a[i+1]) and dele. a[i+1]  

    and it's circular

    make new array where 1st ele is max ele
    So that we have to deal with less max ele

    or just do ans += max(a[i],a[(i+1)%n]) , for all valid i
    ans just sub. vmax
    cout<< ans - vmax(a);
*/
...
void I_still_love_her() {
    ...
    // ll ind = max_element(all(a)) - a.begin();
    
    // vi b(n);

    // fl(i,0,n){
    //     b[i]= a[(ind +i)%n]; // same as rotate(a.begin(), max_element(a.begin(), a.end()), a.end());
    // }
