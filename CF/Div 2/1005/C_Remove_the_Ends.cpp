...
void I_still_love_her()
{
    ...
    vi L(n, 0), R(n, 0);
    vector<bool> Ld(n, false), Rd(n, false);
    ...
    if (n > 0 and a[n - 1] > 0 and Ld[n - 1])
        c1 = L[n - 1];

    if (n > 0 and a[0] < 0 and Rd[0])
        c2 = R[0];

    fl(i, n - 1) if (Ld[i] && Rd[i + 1]) c3 = max(c3, L[i] + R[i + 1]);
