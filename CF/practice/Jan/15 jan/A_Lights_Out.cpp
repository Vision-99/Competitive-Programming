...
void I_still_love_her() {
    ...
    vector<vector<int>> r(3, vector<int>(3, 1));  // Initialize all lights as ON
    
    // Input the number of presses for each button
    fl(i,3) fl(j,3) cin >> a[i][j];

    // Process each cell
    fl(i,3) {
        fl(j,3) {
            // If number of presses is odd, toggle current cell and adjacent cells
            if(a[i][j] & 1) {
                r[i][j] = !r[i][j];
                if(i > 0) r[i-1][j] = !r[i-1][j];
                if(i < 2) r[i+1][j] = !r[i+1][j];
                if(j > 0) r[i][j-1] = !r[i][j-1];
                if(j < 2) r[i][j+1] = !r[i][j+1];
            }
        }
    }
