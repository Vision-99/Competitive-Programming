...
/*
    need to worry abt 4,12,24,32,44
    so need to remove all the 4 for sure [12,32]
    need all the 1, that come before 2 (x1) or ans += min(x1+x3,#2)
    or need this 22222....1113331...
...
void I_still_love_her() {
    ...
    int x13 = 0;
    for (char c : s) if (c == '1' || c == '3') x13++;

    int x2 = 0, p13 = 0, ans = x13;

    for (char c : s) {
        if (c == '2') x2++;
        if (c == '1' || c == '3') p13++;
        ans = max(ans, x2 + x13 - p13);
    }
