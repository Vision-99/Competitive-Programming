...
/*
    ...
    sum array sum a[l...r] = pre[r] - pre[l-1]

    using 1 op.

    delta sum = (r-l+1)*(l+r) - (pre[r] - p[l-1]); we need this to be as max as possisble.
    if you use bf to check all possible l,r it will be O(n^2) TLE alret!

    try using little maths and seperate variable 

    delta sum = ( r^2 + r -pre[r] ) + ( -l^2 + l + p[l-1] )
    1st term depend on r ,2nd on l only
    kepp track of both the term1 and term2 max val , 
    but t1 and t2 are not indepeendent , there is l<=r 
    So ans is Vsum + max_term1 + max_term2 , with l <=r
...
void I_still_love_her() {
    ...
    int ans=INT_MIN,t2 = INT_MIN;
    int sm=0;


    for(int i=1;i<=n;i++){
        // this will make sure l <=r 
        cin>>a[i];
        t2 = max(t2,-1*i*i + i + sm);
        sm += a[i];
        ans = max(ans,i*i + i - sm + t2);
    }
    cout<< vsum(a) + ans;
