...
void I_still_love_her() {
    ...
    for (int i = 0; i < M; ++i) {
        ...
        // Define the 4 cells this 2x2 block would occupy
        // Top-Left, Bottom-Left, Top-Right, Bottom-Right
        pair<int, int> cells[4] = {
            {r, c},
            {r + 1, c},
            {r, c + 1},
            {r + 1, c + 1}
        ...
        // Check if any of these 4 cells are already in the set
        bool overlap = false;
        for (int k = 0; k < 4; ++k) {
            if (occupied_cells.count(cells[k])) {
                overlap = true;
                break;
            }
        }

        // If no overlap, place the block
        if (!overlap) {
            blocks_placed++;
            for (int k = 0; k < 4; ++k) {
                occupied_cells.insert(cells[k]);
            }
        }
    }
