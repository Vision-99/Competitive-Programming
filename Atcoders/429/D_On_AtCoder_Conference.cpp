...
void I_still_love_her() {
    ...
    for(int i=0;i<n;){
        long long x = a[i];
        long long cnt = 0;
        while(i<n && a[i]==x){ ++cnt; ++i; }
        pos.push_back(x);
        w.push_back(cnt);
    }
    int K = (int)pos.size();

    // gaps g_j : number of starting integer s whose first occupied point is pos[j]
    vector<long long> gap(K);
    for(int j=0;j<K;j++){
        long long prev = pos[(j-1+K)%K];
        long long g = (pos[j] - prev + m) % m;
        if(g == 0) g = m;
        gap[j] = g;
    }

    // two-pointer over duplicated w array
    vector<long long> w2(2*K);
    for(int i=0;i<2*K;i++) w2[i] = w[i % K];
    ...
    __int128 ans128 = 0;
    for(int j=0;j<K;j++){
        ans128 += (__int128)gap[j] * (__int128)X[j];
    }
