...
/*
    ...
    each query is just , stack ka size, if zero yes else no
*/
...
void I_still_love_her() {
    ...
    vector<int> bal;   // prefix balances
    vector<int> mn;    // prefix minima
    int cur = 0;       // current (final) balance
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (t == 1) {
            ...
            bal.pb(cur);
            if (mn.empty()) mn.pb(cur);
            else mn.pb(min(mn.back(), cur));
        } 
        else {
            // guaranteed non-empty when t==2
            bal.pop_back();
            mn.pop_back();
            cur = bal.empty() ? 0 : bal.back();
        }

        if (cur == 0 && (mn.empty() || mn.back() >= 0)) cout << "Yes\n";
        else cout << "No\n";
    }
}
