...
ll ceil_sqrt(ll n){
    if(n<=0) return 0;
    ll m=(ll)sqrtl((long double)n);
    if(m*m==n) return m;
    if(m*m<n){
        if((m+1)*(m+1)>=n) return m+1;
    }else{
        if((m-1)*(m-1)<n) return m;
    }
    while(m*m<n) m++;
    while(m>0 && (m-1)*(m-1)>=n) m--;
    return m;
}
...
void I_still_love_her() {
    ll c,d; cin>>c>>d;
    ll ans=0;
    vector<ll> p(19); p[0]=1;
    fle(i,1,18) p[i]=p[i-1]*10;
    fl(k,1,11){
        ll y1=max(c+1,p[k-1]), y2=min(c+d,p[k]-1);
        if(y1>y2) continue;
        ll L=c*p[k]+y1, R=c*p[k]+y2;
        ll a=ceil_sqrt(L), b=floor_sqrt(R);
        if(b>=a) ans+=b-a+1;
    }
    cout<<ans;
