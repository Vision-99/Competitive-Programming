...
void I_still_love_her() {
    ...
    fl(i,0,n){
        int w = a[i][0];
        ll d = (ll)a[i][1] - (ll)a[i][2];
        for (int j = cap; j >= w; --j) {
            if (dp[j-w] != NEG) dp[j] = max(dp[j], dp[j-w] + d);
        }
    }

    ll best = 0;
    fl(j,0,cap+1) if (dp[j] != NEG) best = max(best, dp[j]);
