...
/*
    n even-> both odd-> -1
    n odd-> the one which is odd is the final color of the blocks
...
void I_still_love_her() {
    ...
    char goal = (b&1)?'B':'W'; // even case any one okay, odd case need odd char
    
    for(int i=0;i+1<n;i++){
        if(s[i]!=goal){
            ans.pb(i+1);
            flip(s[i]); // can be done with s[i] ^= 'B' ^ 'W';
            flip(s[i+1]); 
        }
    }
