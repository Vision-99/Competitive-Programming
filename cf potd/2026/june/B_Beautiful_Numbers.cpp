...
/*
    start with ind where we have '1'
    make two pointers l=ind-1, r= ind+1 ( in valid range)
    put min ele in set and check if *st.rbegin() == m ( if yes 1 else 0)
...
void I_still_love_her() {
    ...
    while(m<n){
        if(l <0) st.insert(a[r++]);
        else if(r>=n) st.insert(a[l--]);
        else if(l>=0 and r<n and a[l] < a[r]) st.insert(a[l--]);
        else if(l>=0 and r<n and a[l] > a[r]) st.insert(a[r++]);
        m++;
        if(*st.rbegin()==m) ans+='1';
        else ans+='0';
    }
