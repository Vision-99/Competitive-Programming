...
/*
    ans = (n - Cmn)*(mn+1) + Cmn*mn
    the abvove formula is correct only when there is no order or selection 
    but , ATQ , you have to choose next
    we want our start to be just next of Last min ele. 
    use this for extar --> longest consecutive sequence of elements > mn (circular)
*/
...
void I_still_love_her() {
    ...
    // Find longest consecutive sequence of elements > mn (circular)
    int cnt = 0, mx = 0;
    for (int i = 0; i < 2 * n; i++) {
        int j = i % n;  // Handle circular array
